class Box3D:
    def __init__(self, width, height, depth):
        ...
        self.height = height
        self.depth = depth
        self.vec = [self.width, self.height, self.depth]

    def __add__(self, other):
        return Box3D(*[sum(x) for x in zip(self.vec, other.vec)])

    def __mul__(self, other):
        return Box3D(*[x * other for x in self.vec])

    def __rmul__(self, other):
        return self.__mul__(other)

    def __sub__(self, other):
        return Box3D(*[x[0] - x[1] for x in zip(self.vec, other.vec)])
