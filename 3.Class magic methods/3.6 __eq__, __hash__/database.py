...
class DataBase:
    ...
    def read(self, pk):
        return [y for x in self.dict_db for y in self.dict_db[x] \
                if y.pk == pk][0]
...
db = DataBase("./")
[db.write(Record(*x.split("; ")[:-1], int(x.split("; ")[-1]) )) for x in lst_in]
# [print(k.pk, k.fio, k.descr, k.old, db.dict_db[k]) for k in db.dict_db]
