...
class MotherBoard:
    ...
    def get_config(self):
        mem_str =' '.join([f"{i.name} - {i.volume};" for i in self.mem_slots])[:-1]
        return [f'Материнская плата: {self.name}',
                f'Центральный процессор: {self.cpu.name}, {self.cpu.fr}',
                f'Слотов памяти: {self.total_mem_slots}',
                f'Память: {mem_str}']

mb = MotherBoard("ASRock 970 Pro3 R2.0", CPU("intel core i7", 2800), \
                [Memory("Kingston", 4096), Memory("Kingston", 4096)])
